...
namespace mln
{
	...
	void Record(string path, string line, char parametr = 'c')//Если параметр равен c (change), то файл будет перезаписываться,а если a (add), то добавляться к концу 
	{
		if (parametr == 'a')
		{
	...
	string fulldata(string path, int index, int parameter)//для считывания одного параметра
	{
		char str[150], fline[30];
		ifstream data(path);
		string line, inf;
